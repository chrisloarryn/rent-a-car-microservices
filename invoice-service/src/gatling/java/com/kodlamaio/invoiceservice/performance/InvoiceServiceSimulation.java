package com.kodlamaio.invoiceservice.performance;
...
public class InvoiceServiceSimulation extends Simulation
{
    private final HttpProtocolBuilder httpProtocol = http
            .baseUrl(GatlingSettings.baseUrl())
            .acceptHeader("application/json")
            .userAgentHeader("Gatling invoice-service");

    private final ScenarioBuilder scenario = scenario("invoice_query_flow")
            .exec(http("list_invoices")
                    .get("/api/invoices")
                    .check(status().is(200)))
            .exec(http("invoice_docs")
                    .get("/v3/api-docs")
                    .check(status().is(200)));
